...
function CardComponent({ card, isSelected, onClick }: { card: Card, isSelected: boolean, onClick: () => void }) {
    ...
    return (
        <div 
            ...
            onClick={onClick}
        >
            <div className={`text-2xl font-bold ${isRed ? 'text-red-500' : 'text-black'}`}>
                {card.rank}
                <span className="ml-1">{suitSymbols[card.suit]}</span>
            </div>
            <div className={`text-2xl font-bold self-end transform rotate-180 ${isRed ? 'text-red-500' : 'text-black'}`}>
                {card.rank}
                <span className="ml-1">{suitSymbols[card.suit]}</span>
            </div>
        </div>
